...
import { Text, Keyboard, StyleSheet, View, BackHandler } from "react-native";
...
class NewServerView extends React.Component {
  ...
  onPressServerHistory = (serverHistory) => {
    this.setState({ text: serverHistory?.url }, () =>
      this.submit({
        fromServerHistory: true,
        username: serverHistory?.username,
      })
    );
  };
  ...
  submit = async ({ fromServerHistory = false, username }) => {
    ...
    if (text) {
      ...
      await UserPreferences.setStringAsync(
        `${RocketChat.CERTIFICATE_KEY}-${server}`,
        certificate
      );
  ...
  renderCertificatePicker = () => {
    ...
    return (
      <View style={pagestyles.certificatePicker}>
        <Text style={[pagestyles.chooseCertificateTitle, { color: "black" }]}>
          {certificate
        ...
        >
          <Text style={[pagestyles.chooseCertificate, { color: "blue" }]}>
    ...
    return (
      <>
        <LinearGradient
          ...
          locations={[0, 0.5, 0.9]}
        >
          <Text style={[pagestyles.title, { color: "black" }]}>
