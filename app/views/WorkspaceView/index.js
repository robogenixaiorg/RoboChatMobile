      ...
      >
        <View style={styles.alignItemsCenter}>
          <ServerAvatar
            theme={theme}
            url={server}
            image={Assets_favicon_512?.url ?? Assets_favicon_512?.defaultUrl}
          />
          <Text style={[styles.serverName, { color: "white" }]}>
            {Site_Name}
          </Text>
          <Text style={[styles.serverUrl, { color: "black" }]}>{Site_Url}</Text>
        </View>
        {showLoginButton ? (
          <Button
            title={I18n.t("Login")}
