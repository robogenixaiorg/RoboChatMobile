...
class Sidebar extends Component {
  ...
  async setIsAdmin() {
    ...
    try {
      if (roles) {
        ...
        const permissionsFiltered = await permissionsCollection
          .query(Q.where("id", Q.oneOf(permissions)))
          .fetch();
        const isAdmin = permissionsFiltered.reduce(
          (result, permission) =>
  ...
  renderAdmin = () => {
    ...
    const routeName = isMasterDetail
      ? "AdminPanelView"
  ...
  renderCustomStatus = () => {
    ...
    return (
      <SidebarItem
        ...
        right={
          <CustomIcon name="edit" size={25} color={themes[theme].sidebarText} />
        }
